...
class LinkedList:
    ...
    def tail(self, data=None):
        ...
        self.__tail.set_next(node) # this should connect the tail to the next node
        self.__tail = node
    ...
    def append(self, data):
        """
        Adds a new node to the tail, sets a pointer from previous tail to next tail
        """
        # adds to tail
    ...
    def pop(self):
        """
        Removes current head from the linked list if there is one present
        """
        # case 1: no data
        # case 2: 1 item
        # case 3: 2+ items
